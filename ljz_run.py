...
msg_in = ["python run.py", "Test", "./testyaml/mss_demo_caps.yaml", "1"]

def generate_phone_num(i_num):
    if msg_in[1] == "Test":
        return str(19900000001 + i_num)
    elif msg_in[1] == "CC02":
        return "0"+str(30000000801 + i_num)
    else:
        print ("No such environment. %s" % msg_in[1])
        sys.exit(1)
...
def worker(p_stop_signal, result_queue):
    ...
    u = Scene(msg_in[2], user_pool, p_stop_signal)
    u.run()
...
if __name__ == '__main__':
    if len(msg_in) < 4:
        ...
        e.g.: python run.py CC02 "./testyaml/login.yaml" 1
        """
        sys.exit()
    w1 = None
    try:
