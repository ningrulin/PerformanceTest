...
class V3GetDeviceInfoAction(ExecuteAction):
    ...
    def _execute_action(self):
        try:
            ...
            if http_response.status_code == 200:
                ...
                self.user_pool.cache_lock.release()
                self._logger.info("V3GetDeviceInfoAction success!")
                ...
                self.user_pool.online_user_phone_num_list.appendleft(self.executor_phone_num)
                self.user_pool.cache_lock.release()
                self._logger.info("V3GetDeviceInfoAction Fail!")
                self._logger.info("Sleep %s s after action!" % sleep_after)
    ...
    def _execute_http_request(self):
        ...
        url = Constant.CAS_HOST_V3 + "/mhauth/account"
        data = {
            "version": "3.3.0"
        }
        ...
        r = execute_http_client.request(method=method, url=url, name="/mhauth/account", catch_response=False, headers=headers, data = data)
        print r.text
        ...
        mod_user_info_dict['userID'] = str(mod_user_info_dict['userid'])
