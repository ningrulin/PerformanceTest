...
class CreateConf6Action(ExecuteAction):
    ...
    def _execute_action(self):
        try:
            ...
            if not sipcall.make_call(caller_sip_number_list, str(conf_id),
                                     SipMqConstants.SIP_SERVER, SipMqConstants.SIP_PASSWORD,
            ...
            if not self.only_self:
                ...
                if len(self.received_call_notify_num_list) < all_in_conf_mem_num :
                    ...
                    ret_list = [item for item in self.received_inconf_info if item not in all_in_conf_mem_num] + [item for item in all_in_conf_mem_num if item not in self.received_inconf_info]
                    self._logger_ar.debug("creatconf error" + "cas::Expected to receive call_notify Quantity:" + str(all_in_conf_mem_num) +
                                       " Actually received call_notify Quantity:" +
                                       str(len(self.received_call_notify_num_list)) +
                                       " self.received_call_notify_num_list" +
                                       str(self.received_call_notify_num_list) + " Conference ID: " + str(self.conf_id) +
                                       "not in conf list"  + str(ret_list))
                    return False
            ...
            if self.flage:
                ...
                self._logger_ar.debug("The conference was created successfully!")
            else:
                self._logger.debug("The conference was created failed!")
                self._logger_ar.debug("The conference was created failed!")
            self.properly_executed = True

        except:
            self._logger.error("Return False because of except.")
            self._logger_ar.debug("The conference was created except!")
