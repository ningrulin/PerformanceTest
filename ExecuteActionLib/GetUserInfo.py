...
class GetUserInfoAction(ExecuteAction):
    ...
    def _execute_http_request(self):
        ...
        url = Constant.HOST + "/mhuser/display/profile"
        method = 'GET'
        headers = {'authorization': self.executor_obj.user_info_dict["basicToken"],
                   "Content-Type": "application/json; charset=utf-8"}
        execute_http_client = HttpClient(url)
        r = execute_http_client.request(method=method, url=url, name="/mhuser/display/profile", catch_response=False, headers=headers)
        print r.text
