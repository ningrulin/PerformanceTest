...
class CreateConfAction(ExecuteAction):
    ...
    def _analyze_notify(self, **kwargs):
        if "message" in kwargs:
            ...
            if event_name == "mx.conf3.invite" and event_body.get(
                    "id", "") == self.conf_id:
                # self.action_resource_lock.acquire()
                ...
                if kwargs["user_num"] != self.chairman_phone_num:
                    ...
                    execute_http_client = HttpClient(url)
                    data = {"device": "box"}
                    r = execute_http_client.request(method="POST", url=url, name="/callme", catch_response=False, data=json.dumps(data),
                                                    headers=headers)
                    return r

            elif event_name == "mx.conf3.member" and event_body.get(
                    "id", "") == self.conf_id:
                event_uid = event_body.get("uid", "")
                event_mid = event_body.get("mid", "")
